...
function AircraftModal({ mode, aircraft, onClose, onSave, isSaving, airlines }: AircraftModalProps) {
  ...
  const [airlineId, setAirlineId] = useState(aircraft?.airlineId?.toString() ?? "")
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Hãng hàng không sở hữu</Label>
            {mode === "create" ? (
              <Select value={airlineId} onValueChange={setAirlineId} required>
                <SelectTrigger>
              ...
              <Input value={aircraft?.airlineName} disabled className="bg-muted/50" />
            )}
          </div>
...
export default function AircraftsPage() {
  ...
  const { data: aircrafts = [], isLoading: isAircraftsLoading } = useQuery({
    queryKey: ["admin-aircrafts"],
    queryFn: aircraftService.getAll,
  })

  const { data: airlines = [] } = useQuery({
    queryKey: ["admin-airlines-compact"],
    queryFn: airlineService.getAll,
  })

  const createMutation = useMutation({
    mutationFn: (data: CreateAircraftRequest) => aircraftService.create(data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-aircrafts"] }); setModalMode(null) },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: UpdateAircraftRequest }) => aircraftService.update(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-aircrafts"] }); setModalMode(null); setEditingAircraft(null) },
  })

  const deleteMutation = useMutation({
    mutationFn: (id: number) => aircraftService.delete(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-aircrafts"] }); setDeleteConfirmId(null) },
  })
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-bold tracking-tight">Quản lý máy bay</h2>
            ...
            {filtered.length === 0 && !isAircraftsLoading ? (
              ...
              filtered.map(aircraft => (
                <TableRow key={aircraft.id}>
                  ...
                  <TableCell className="font-medium">{aircraft.airlineName}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
